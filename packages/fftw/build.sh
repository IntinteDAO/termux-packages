...
termux_step_configure() {
	...
	sed -e "s|\(IMPORTED_LOCATION_NONE\).*|\1 \"$TERMUX_PREFIX/lib/libfftw3.so.3\"|" -i build/FFTW3LibraryDepends.cmake
	...
	for target in "${build_targets[@]}"; do
		( # Configure each target's build directory with the appropriate build flags
			...
			# Force static OpenMP linking: replace -lomp with -l:libomp.a
			# in libtool so that even relinks during install use static libomp
			sed -i -e 's/-lomp/-l:libomp.a/g' libtool
		)
...
termux_step_make(){
	...
	for target in "${build_targets[@]}"; do
		...
		# Patch .la files to force static libomp before install relinks
		find "${ORIG_TERMUX_PKG_BUILDDIR}-${target}" -name '*.la' -exec \
			sed -i -e 's/-lomp/-l:libomp.a/g' {} +
	done
}
...
termux_step_make_install(){
	local target
	for target in "${build_targets[@]}"; do
		make --directory "${ORIG_TERMUX_PKG_BUILDDIR}-${target}" install
	done

	# install missing FFTW3LibraryDepends.cmake
	mkdir -p "${TERMUX_PREFIX}/lib/cmake/fftw3/"
	cp -v "${TERMUX_PKG_SRCDIR}/build/FFTW3LibraryDepends.cmake" "${TERMUX_PREFIX}/lib/cmake/fftw3/"
}
